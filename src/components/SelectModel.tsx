...
export const SelectModel = () => {
  ...
  const car = useCar();
  const { data: modelPages } = UsegetPages(car.isCar);
  ...
  const selectModel = async (model: number) => {
    car.setModel(model);
    const { data } = await UsegetPage(model);
    setPage(data);
    onClose();
  };
  ...
  return (
    <div className="xl:tw-w-80 tw-w-full tw-relative tw-z-[2]">
      <div
        onClick={OnOpen}
      ...
      >
        <span className="tw-text-white tw-text-xl tw-pr-[60px] tw-text-nowrap">
          {page?.title.rendered || "Выберите модель"}
        </span>
      ...
      {open && (
        ...
        >
          <ul className="tw-scrollbar-none tw-min-h-5 tw-max-h-60 tw-overflow-y-scroll tw-flex tw-flex-col">
            {!car.isCar ? "" : modelPages.map(({ id, title: { rendered } }) => (
              <li
